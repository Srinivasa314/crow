...
use crate::ast::*;
use crate::types::{IntKind, StructInfo, Type, INT};
...
/// Why a call-site unification failed.
enum UnifyErr {
    Mismatch,
    /// The argument was `nil`, which cannot pin down a type parameter.
    Nil,
}
...
fn unify(decl: &Type, actual: &Type, solved: &mut [Option<Type>]) -> Result<(), UnifyErr> {
    match (decl, actual) {
        (Type::Param(i), _) => match actual {
            Type::Nil => Err(UnifyErr::Nil),
            Type::Unit | Type::Unknown => Err(UnifyErr::Mismatch),
            _ => match &solved[*i as usize] {
                Some(t) if t == actual => Ok(()),
                Some(_) => Err(UnifyErr::Mismatch),
                None => {
...
impl Checker {
    fn show(&self, t: &Type) -> String {
        format!("{}", t.display(&self.structs, &self.type_params))
    }
    ...
    fn collect_structs(&mut self, program: &Program) -> CResult<()> {
    ...
    fn resolve_type(&self, te: &TypeExpr) -> CResult<Type> {
        match te {
            TypeExpr::Named(name, args, line) => {
                ...
                match self.struct_ids.get(name) {
                    Some(&id) => {
                        ...
                        Ok(Type::Struct(id, targs))
                    }
                    None => Err(format!("{line}: unknown type '{name}'")),
                }
            }
            ...
            Stmt::Let { name, ann, init, line, local, ty } => {
                ...
                let final_ty = match want {
                    ...
                    None => match &init.ty {
                        Type::Nil => {
                            return Err(format!(
                                "{line}: cannot infer the type of 'nil'; add a type annotation"
                            ))
                        }
                        Type::Unit => {
    ...
    fn require_assignable(&self, target: &Type, value: &Type, line: u32) -> CResult<()> {
        if target == value || (*value == Type::Nil && target.accepts_nil()) {
            Ok(())
        } else {
            Err(format!(
    ...
    fn check_expr(&mut self, e: &mut Expr, expected: Option<&Type>) -> CResult<()> {
        ...
        match &mut e.kind {
            ...
            ExprKind::Nil => {
                e.ty = match expected {
                    Some(t) if t.accepts_nil() => t.clone(),
                    _ => Type::Nil,
                };
            }
            ...
            ExprKind::Binary(op, lhs, rhs) => {
                ...
                match op {
                    ...
                    BinOp::Eq | BinOp::Ne => {
                        ...
                        let want = if lty == Type::Nil { None } else { Some(&lty) };
                        self.check_expr(rhs, want)?;
                        ...
                        let ok = lhs.ty == rhs.ty
                            || (lhs.ty == Type::Nil && rhs.ty.accepts_nil())
                            || (rhs.ty == Type::Nil && lhs.ty.accepts_nil());
                        if !ok || lhs.ty == Type::Unit {
                            return Err(format!(
                                "{}: cannot compare {} with {}",
                                line,
                                self.show(&lhs.ty),
            ...
            ExprKind::If { cond, then, els } => {
                ...
                // A nil then-branch cannot pin the result type; let the else
                // branch see the outer expectation instead.
                let want = if then.ty == Type::Nil {
                    expected.cloned()
                } else {
                    Some(then.ty.clone())
                };
                self.check_expr(els, want.as_ref())?;
                let ty = if then.ty == els.ty {
                    then.ty.clone()
                } else if then.ty == Type::Nil && els.ty.accepts_nil() {
                    els.ty.clone()
                } else if els.ty == Type::Nil && then.ty.accepts_nil() {
                    then.ty.clone()
                } else {
                    return Err(format!(
                ...
                for el in elems.iter_mut() {
                    ...
                    match &elem_ty {
                        None => {
                            if el.ty == Type::Nil {
                                return Err(format!(
                                    "{}: cannot infer array element type from 'nil'",
                                    el.line
                                ));
                            }
                            elem_ty = Some(el.ty.clone());
                        }
                ...
                for (fname, value, index) in fields.iter_mut() {
                    ...
                    *index = i as u32;
                    ...
                    if !want.has_param() {
                        ...
                        match unify(&decl[i].1, &value.ty, &mut solved) {
                            Ok(()) => {}
                            Err(UnifyErr::Nil) => {
                                return Err(format!(
    ...
    fn infer_call(
        ...
        for (arg, pty) in args.iter_mut().zip(params) {
            ...
            if !want.has_param() {
                ...
                match unify(pty, &arg.ty, &mut solved) {
                    Ok(()) => {}
                    Err(UnifyErr::Nil) => {
                        return Err(format!(
                            "{}: cannot infer a type parameter from 'nil'; \
                             pass a typed value or annotate the surrounding context",
                            arg.line
                        ))
                    }
                    Err(UnifyErr::Mismatch) => {
    ...
    fn check_builtin(
        ...
        let ty = match b {
            ...
            Builtin::Btos => {
                ...
                if args[0].ty != want && args[0].ty != Type::Nil {
                    return Err(format!(
...
mod tests {
    ...
    #[test]
    fn nil_adopts_expected_type() {
        let (program, _) = check_src(
            r#"struct P { v: int }
               fn main() { let p: P = nil; }"#,
        )
        .unwrap();
        let Stmt::Let { init, .. } = &program.funcs[0].body.stmts[0] else { panic!() };
        assert_eq!(init.ty, Type::Struct(0, vec![]));
    }
    ...
    #[test]
    fn name_errors() {
        assert!(err_in_main("let x: Foo = nil;").contains("unknown type 'Foo'"));
    ...
    fn type_mismatch_errors() {
        ...
        assert!(err_in_main("let x: int = nil;").contains("type mismatch"));
        ...
        assert!(err_in_main("let x = nil == 1;").contains("cannot compare"));
    ...
    #[test]
    fn inference_errors() {
        assert!(err_in_main("let x = nil;").contains("cannot infer the type of 'nil'"));
        assert!(err_in_main("let x = [];").contains("cannot infer the type of an empty array"));
        assert!(err_in_main("let x = [nil];").contains("cannot infer array element type"));
    ...
    fn generic_errors() {
        ...
        assert!(err("fn f<T>(x: T): bool { return x == nil; } fn main() { f(1); }")
            .contains("cannot compare values of generic type T"));
        ...
        assert!(err("fn f<T>(x: T) { let y: T = nil; } fn main() { f(1); }")
            .contains("type mismatch: expected T, found nil"));
        // Inference failures.
        assert!(err("fn f<T>(x: T) { } fn main() { f(nil); }")
            .contains("cannot infer a type parameter from 'nil'"));
        ...
        assert!(
            err("struct P<T> { v: T } fn main() { let p = P { v: nil }; }")
                .contains("cannot infer a type parameter from 'nil'")
        );
        ...
        assert!(err("struct P<T> { v: T } fn main() { let p: P<int, int> = nil; }")
            .contains("expects 1 type argument(s), got 2"));
        assert!(err("struct P<T> { v: T } fn main() { let p: P = nil; }")
            .contains("expects 1 type argument(s), got 0"));
        assert!(err("struct P { v: int } fn main() { let p: P<int> = nil; }")
        ...
        assert!(err(
            "struct P<T> { v: T } \
             fn main() { let a: P<int> = nil; let b: P<u8> = nil; a = b; }"
        )
    ...
    fn if_expression_rules() {
        let (_, checked) = check_src(
            r#"struct P { v: int }
               fn main() {
                   ...
                   let c = if true { P { v: 1 } } else { nil };
                   let d = if true { nil } else { P { v: 1 } };
        ...
        assert_eq!(locals[3], Type::Struct(0, vec![]));
        assert_eq!(locals[4], Type::Int(IntKind::U8));
        ...
        assert!(err_in_main("let x = if true { nil } else { nil };")
            .contains("cannot infer the type of 'nil'"));
    }
